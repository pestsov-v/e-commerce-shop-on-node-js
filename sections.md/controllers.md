...
```
Где:<br/>
**1. Визуализация страницы "Курсов":** <br/>
Блок кода отвечающий за визуализацию страницы "Курсов": <br/>
```node
res.render('courses', {})
```
Где:<br/>
...
Код, который передаёт данные для последующей визуализации страницы одного курса: <br/>
```node
res.render('course', {
      layout: 'empty',
      title: `Курс ${course.title}`,
...
```
Где: <br/>
- `User.findById()` - `req.user._id` <br/>
- `toChange` - `name` `req.body.name` <br/>
...
Где: <br/>
- `await req.user` - <br/>
- `populate()` - `cart.items.courseId` <br/>
- `execPopulate()` - <br/>
...
Где: <br/>
- `computePrice(courses) {}` -  <br/>
- `courses.reduce(() => {})` - `total` `course` <br/>
- `total += course.price * course.count` - <br/>
  - `course.price` - <br/>
  - `course.count` - <br/>
...
```
Где: <br/>
- `Course.findById()` - `req.body.id` <br/>
...
```
Где: <br/>
- `req.user.addToCart()` - `course` <br/>
- `res.redirect()` - `'/card'` <br/>
...
```
Где: <br/>
- `userSchema.methods.addToCart` - <br/>
- `function(course) {}` - <br/>
- `const items = [...this.cart.items]` - <br/>
- `const idx = items.findIndex(c => {})` - <br/>
- `return c.courseId.toString() === course._id.toString()` - <br/>
- `if (idx >= 0) {}` - <br/>
- `items[idx].count = items[idx].count + 1` - <br/>
- `items.push({})` - <br/>
- `courseId` - `course._id` <br/>
- `count` - `1` <br/>
- `this.cart` - `{items}` <br/>
- `return this.save()` - <br/>
...
```
Где: <br/>
- `req.user.removeFromCart()` - `req.params.id` <br/>
...
```
Где: <br/>
- `userSchema.methods.removeFromCart` - <br/>
- `function()` - `id` <br/>
- `let items = [...this.cart.items]` - <br/>
- `const idx = items.findIndex(c => c.courseId.toString() === id.toString())` - <br/>
- `if (items[idx].count === 1) {}` - <br/>
- `items[idx].count--` - <br/>
- `this.cart` - `{items}` <br/>
- `this.save()` - <br/>
...
```
Где: <br/>
- `mapCartItems()` - `user.cart` <br/>
- `cart` - <br/>
  - `courses` -  <br/>
  - `price` - `computePrice(courses)` <br/>
- `res.status(200).json(cart)` - <br/>
...
```
Где: <br/>
- `Order.find({})` - `'user.userId': req.user._id` <br/>
    - `user.userId` - <br/>
    - `req.user._i` - <br/>
- `populate()` - `user.userId` <br/>
...
```
Где: <br/>
- `orders: orders.map(o => {})` - <br/>
- `...o._doc` -  <br/>
- `price` - `o.courses.reduce((total, c) => {})` <br/>
- `o.courses.reduce((total, c) => {})` - <br/>
- `total += c.count * c.course.price` - <br/>
...
**2. Подсчёт всех курсов и суммы этих курсов:** <br/>
Блок кода: <br/>
```node
...
    const courses = user.cart.items.map(i => ({
...
```
Где: <br/>
- `user.cart.items.map(i => ({})` - <br/>
- `count` - `i.count` <br/>
- `course` - `{...i.courseId._doc}` <br/>
...
```
Где: <br/>
- `req.user.clearCart()` - <br/>
- `res.redirect()` - `'/orders'` <br/>
...
```
Где: <br/>
- `userSchema.methods.clearCart` - <br/>
- `function() {}` - <br/>
- `this.cart` - `{items: []}` <br/>
- `return this.save()` - <br/>
