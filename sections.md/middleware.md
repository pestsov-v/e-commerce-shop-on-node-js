...
- [Промежуточные обработчики](#промежуточные-обработчики)
  ...
  - [- `<input type="hidden" name="_csrf" value="{{csrf}}">` - токен промежуточного обработчика csurf, который проверяется по сеансу пользователя. <br/>](#--input-typehidden-name_csrf-valuecsrf---токен-промежуточного-обработчика-csurf-который-проверяется-по-сеансу-пользователя-)
