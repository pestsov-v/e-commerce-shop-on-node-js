...
- [Модели](#модели)
  ...
  - [Реализация модели курса "Course" в контроллерах приложения](#реализация-модели-курса-course-в-контроллерах-приложения)
    - [В контроллере "Добавить курс" с типом запроса `post` маршрутизатора `/add`](#в-контроллере-добавить-курс-с-типом-запроса-post-маршрутизатора-add)
...
### В контроллере "Добавить курс" с типом запроса `post` маршрутизатора `/add`

--- 

Контроллер с типом запроса `post` маршрутизатора `/add` реализовывает бизнес-логику создание нового курса. Блок кода, отвечающий за даную логику:
```node
...
Подробнее о данном контролере смотрите [Тип запроса `post` по маршруту `'/add'`](controllers.md#тип-запроса-post-по-маршруту-add)
